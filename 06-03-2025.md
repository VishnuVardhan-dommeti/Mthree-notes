...
Create a Deployment for application pods.

Expose it using a Service.

Use an Ingress Controller for domain-based routing.
...
Frontend (React) → Runs inside a pod.

Backend (Django) → Runs in a separate pod.

Database (MySQL) → Uses Persistent Volume for data storage.

LoadBalancer Service → Exposes frontend to users.

Ingress Controller → Routes traffic based on domains.
